...
PNG ubcify(PNG image) {
  //   for (int y = 0; y < image.height(); y = y + 5){
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double distToYellow = colordist(*pixel, ubcYellow );
      double distToBlue = colordist(*pixel, ubcBlue);

      if (distToBlue >= distToYellow){
        pixel->r = 247;
        pixel->g = 184;
        pixel->b = 0;
      } else {
  ...
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      ...
      if (pixel->r == 255 & pixel->g == 255 & pixel->b == 255) {
        ...
        RGBAPixel* pixel2 = firstImage.getPixel(x, y);
        if (pixel2->r > 215) {
          pixel2->r = 255;
        } else {
          pixel2->r += 40;
